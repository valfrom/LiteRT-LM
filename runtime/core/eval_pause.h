...
class EvalPauseController {
 public:
  ...
  void Resume();
  void Notify();
  absl::Status WaitIfPaused(const std::atomic_bool* cancelled = nullptr);
  bool IsPaused() const;
...
class EvalPauseCancellationScope {
 public:
  explicit EvalPauseCancellationScope(const std::atomic_bool* cancelled);
  ~EvalPauseCancellationScope();

 private:
  const std::atomic_bool* previous_;
};
